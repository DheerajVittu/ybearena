...
import { Phone, MapPin, User, Calendar, Clock, Tag, AlertCircle, ChevronDown } from "lucide-react";
...
export default function MainPage() {
  ...
  useEffect(() => {
    if (startTime && endTime && errors.startTime === "" && errors.endTime === "" && errors.date === "") {
      const isOverlapping = bookings
        ...
        .some((slot) => {
          ...
          const adjustedSlotEndMinutes = slotEnd === "00:00" ? 24 * 60 : slotEndMinutes;
          
          // Check for overlap
          return startMinutes < adjustedSlotEndMinutes && adjustedEndMinutes > slotStartMinutes;
        });
  ...
  const handleBookNow = () => {
    ...
    if (!validateTimes(startTime, endTime, date)) {
      alert("Please correct the time selections");
      return;
    }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-5 mb-6">
            ...
            <div className="mb-5">
              ...
              <input
                type="date"
                className={`w-full p-4 border-2 ${errors.date ? 'border-red-500' : 'border-gray-200'} rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all`}
                value={date}
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="relative" ref={startDropdownRef}>
                  ...
                  <button
                    ...
                    className={`w-full p-4 border-2 ${errors.startTime ? 'border-red-500' : 'border-gray-200'} rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all flex justify-between items-center bg-white`}
                  >
                    <span className={`${startTime ? 'text-gray-800' : 'text-gray-500'}`}>
                      {formatTimeDisplay(startTime)}
                    </span>
                    <ChevronDown size={16} className={`text-gray-500 transition-transform ${showStartDropdown ? 'rotate-180' : ''}`} />
                  </button>
                      ...
                      {getStartTimeOptions().map((time) => (
                        <button
                          ...
                          className={`w-full px-4 py-3 text-left hover:bg-blue-50 transition-colors ${
                            startTime === time ? 'bg-blue-100 text-blue-700 font-medium' : 'text-gray-700'
                        ...
                        >
                          <div>
                            ...
                            <span className="text-xs text-gray-500 ml-2">{getTimePeriod(time)}</span>
                          </div>
                          {startTime === time && (
                ...
                <div className="relative" ref={endDropdownRef}>
                  ...
                  <button
                    ...
                    disabled={!startTime}
                    className={`w-full p-4 border-2 ${errors.endTime ? 'border-red-500' : 'border-gray-200'} ${!startTime ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'} rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all flex justify-between items-center`}
                  >
                    <span className={`${endTime ? 'text-gray-800' : 'text-gray-500'}`}>
                      {formatTimeDisplay(endTime)}
                    </span>
                    <ChevronDown size={16} className={`text-gray-500 transition-transform ${showEndDropdown ? 'rotate-180' : ''}`} />
                  </button>
                    ...
                    <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-64 overflow-y-auto">
                      {getEndTimeOptions().map((time) => (
                        <button
                          ...
                          className={`w-full px-4 py-3 text-left hover:bg-blue-50 transition-colors ${
                            endTime === time ? 'bg-blue-100 text-blue-700 font-medium' : 'text-gray-700'
                          } border-b border-gray-100 last:border-b-0 flex justify-between items-center`}
                        >
                          <div>
                            <span className="font-medium">{formatTimeDisplay(time)}</span>
              ...
              {(errors.startTime || errors.endTime) && (
                <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-start">
                    <AlertCircle className="mr-2 text-red-600 mt-0.5" size={16} />
                    <div>
                      <h4 className="font-semibold text-red-700 text-sm">Time Selection Rules:</h4>
                      <ul className="text-xs text-red-600 mt-1 space-y-1">
                        ...
                        <li>• Midnight (12:00 AM) is available only in end time for today</li>
                        <li>• Midnight (12:00 AM) represents the end of the day</li>
                        <li>• Overnight bookings (e.g., 10:00 PM to 12:00 AM) are allowed</li>
                      </ul>
                    </div>
                  </div>
                </div>
              ...
              {startTime && endTime && errors.startTime === "" && errors.endTime === "" && (
                <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex items-center">
                    <Clock className="mr-2 text-green-600" size={16} />
                    <div>
                      <span className="text-green-700 font-medium">
                        Selected: {formatTimeDisplay(startTime)} to {formatTimeDisplay(endTime)}
                      </span>
            ...
            {/* INVOICE SECTION */}
            {startTime && endTime && amount > 0 && errors.startTime === "" && errors.endTime === "" && (
              <div className="mt-6 p-5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-100">
                ...
                <div className="space-y-3 mb-4">
                  {timeSlots.map((slot, index) => (
                    ...
                    >
                      <div>
                        <div className="font-medium text-gray-800">{slot.period}</div>
                        <div className="text-xs text-gray-500">
                ...
                <div className="mt-3 text-xs text-gray-500 text-center">
                  • No Cancellation and Refunds • All taxes included • No hidden charges 
                </div>
              </div>
            )}
            ...
            <button
              ...
              disabled={!startTime || !endTime || amount === 0 || errors.startTime !== "" || errors.endTime !== "" || errors.date !== ""}
              className={`w-full py-4 mt-6 text-white font-bold rounded-xl shadow-lg transition-all transform ${
                !startTime || !endTime || amount === 0 || errors.startTime !== "" || errors.endTime !== "" || errors.date !== ""
                  ? 'bg-gray-400 cursor-not-allowed'
                  : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 active:scale-95'
              }`}
            >
              {!startTime || !endTime || errors.startTime !== "" || errors.endTime !== "" || errors.date !== "" 
          ...
          <div className="bg-white rounded-2xl shadow-xl p-5">
            <h2 className="text-xl font-bold text-gray-800 mb-4">📅 Today's Bookings</h2>
            ...
            {bookings.filter((slot) => slot.SlotDate === date).length === 0 ? (
              <div className="text-center py-8 bg-gradient-to-br from-green-50 to-emerald-100 rounded-xl border-2 border-green-200">
                ...
                <p className="font-bold text-green-700 text-lg">All Slots Available!</p>
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="text-sm text-red-600 flex items-center mt-1">
                            ...
                            Booked • {slot.StartTime < "12:00" ? "Morning" : slot.StartTime < "18:00" ? "Afternoon" : "Evening"}
                          </div>
                        </div>
        ...
        <div className="container mx-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-lg font-semibold text-blue-300">Thotakura Rahul Yadav</p>
            </div>
          ...
          <div className="mt-6 pt-4 border-t border-gray-700 text-center text-gray-400">
            <p>© {new Date().getFullYear()} YBR 360 Arena. All rights reserved.</p>
